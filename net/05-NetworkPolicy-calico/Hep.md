...
на(|с) сетевые интерфейсы хоста. Т.е. при помощи calico можно построить почти полноценный
...
Но там где автоматика, там могут возникать не предвиденные ситуации. Поэтому я ограничусь
ссылкой на пример в 
...
Тогда в первая политика будет выглядеть так:
...
Для доступа к этим ресурсам я напишу отдельную политику, с большим приоритетом:
...
**По умолчанию все NodePort по умолчанию открыты.**
